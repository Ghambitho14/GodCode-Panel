...
export const AdminPage = ({ companyName, logoUrl, userEmail: initialEmail, primaryColor, storefrontMenuUrl = null }) => {
  ...
  const isTeamUserCeo = (u) => String(u?.role ?? '').toLowerCase() === 'ceo';
  ...
  const openTeamEditModal = (u) => {
    if (isTeamUserCeo(u)) {
      showNotify('El usuario CEO no se puede editar desde el equipo.', 'error');
      return;
    }
    setTeamUserToEdit(u);
    setTeamForm({
  ...
  const handleSaveTeamUser = async (e) => {
    ...
    if (isEdit && isTeamUserCeo(teamUserToEdit)) {
      showNotify('El usuario CEO no se puede editar.', 'error');
      return;
    }
  ...
  const handleDeleteTeamUser = async () => {
    ...
    if (isTeamUserCeo(teamUserToDelete)) {
      showNotify('No se puede eliminar un usuario con rol CEO.', 'error');
