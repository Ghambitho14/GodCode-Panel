	...
	const patchCatalogInventoryLink = useCallback(
		async (link, inventoryItemId) => {
			...
			try {
				...
				if (!getRes.ok) throw new Error(data.error || "GET");
				...
				const patchRes = await fetch("/api/tenant-branch-delivery-enabled", {
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
					credentials: "include",
					body: JSON.stringify({ branchId, [key]: next }),
				});
				const patchData = await patchRes.json().catch(() => ({}));
				if (!patchRes.ok) throw new Error(patchData.error || "PATCH");
