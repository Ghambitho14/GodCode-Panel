-- Restaura channel = 'salon' para mesas (regresión en 20260621140000_shift_sequence_per_channel).
...
AS $$
  SELECT CASE
    ...
    WHEN coalesce(nullif(btrim(p_channel), ''), 'pickup') IN ('salon', 'mesa') THEN 'mesa'
...
-- Corregir mesas ya creadas con channel pickup por la regresión.
UPDATE public.orders o
SET channel = 'salon'
WHERE o.channel = 'pickup'
  AND coalesce(o.delivery_fee, 0) = 0
  AND o.delivery_address IS NULL
  AND o.handoff_code IS NULL
  AND lower(btrim(coalesce(o.client_name, ''))) NOT IN ('retiro', 'delivery')
  AND (
    lower(btrim(coalesce(o.client_name, ''))) IN ('salón', 'salon')
    OR (
      btrim(coalesce(o.client_rut, '')) = '1-9'
      AND public.normalize_cl_phone_digits(o.client_phone)
        = public.normalize_cl_phone_digits('+56 9 0000 0000')
    )
  );
