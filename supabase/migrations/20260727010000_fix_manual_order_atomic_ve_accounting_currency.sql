-- Align create_manual_order_atomic_v1 accounting currency with the panel:
-- Venezuela (country VE) always uses USD for orders.total / p_currency,
-- even when branches.currency is VES (local settlement rails).
...
begin
  ...
  select b.company_id,
    case
      when lower(btrim(coalesce(b.country, ''))) in ('ve', 'venezuela') then 'USD'
      else upper(coalesce(nullif(btrim(b.currency), ''), 'CLP'))
    end
