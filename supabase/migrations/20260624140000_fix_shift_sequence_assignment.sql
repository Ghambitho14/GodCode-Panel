-- Fix shift_sequence assignment: match INSERT row values, atomic counter per (shift, group).

CREATE TABLE IF NOT EXISTS public.shift_order_sequences (
  shift_id uuid NOT NULL REFERENCES public.cash_shifts(id) ON DELETE CASCADE,
  sequence_group text NOT NULL,
  last_sequence int NOT NULL DEFAULT 0,
  PRIMARY KEY (shift_id, sequence_group)
);

-- Renumerar particiones con duplicados (colisiones en orders_shift_group_sequence_uidx).
WITH affected_partitions AS (
  SELECT
    o.shift_id,
    public.order_sequence_group(
      o.channel, o.client_name, o.delivery_fee, o.delivery_address, o.handoff_code
    ) AS grp
  FROM public.orders o
  WHERE o.shift_id IS NOT NULL AND o.shift_sequence IS NOT NULL
  GROUP BY 1, 2, o.shift_sequence
  HAVING count(*) > 1
),
ranked AS (
  SELECT
  ...
  WHERE EXISTS (
    ...
    WHERE ap.shift_id = o.shift_id
      AND ap.grp = public.order_sequence_group(
        o.channel, o.client_name, o.delivery_fee, o.delivery_address, o.handoff_code
      )
  )
)
...
FROM ranked r
WHERE o.id = r.id
  AND o.shift_sequence IS DISTINCT FROM r.new_seq;

-- Backfill contadores desde pedidos existentes.
...
SELECT
  ...
  max(o.shift_sequence) AS last_sequence
FROM public.orders o
WHERE o.shift_id IS NOT NULL AND o.shift_sequence IS NOT NULL
GROUP BY 1, 2
ON CONFLICT (shift_id, sequence_group) DO UPDATE
SET last_sequence = GREATEST(
  public.shift_order_sequences.last_sequence,
...
BEGIN
  ...
  v_sequence_group := public.order_sequence_group(
    v_channel, p_client_name, v_row_delivery_fee, v_row_delivery_address, v_handoff
  );
  ...
  IF v_shift_id IS NOT NULL THEN
    INSERT INTO public.shift_order_sequences (shift_id, sequence_group, last_sequence)
    VALUES (v_shift_id, v_sequence_group, 1)
    ON CONFLICT (shift_id, sequence_group)
    DO UPDATE SET last_sequence = public.shift_order_sequences.last_sequence + 1
    RETURNING last_sequence INTO v_shift_sequence;
  END IF;
