-- Numeración estable de shift_sequence por tipo (mesa / retiro / delivery) dentro de cada turno.
-- El número se asigna al crear el pedido y no cambia cuando otros se cierran.

CREATE OR REPLACE FUNCTION public.order_sequence_group(
  p_channel text,
  p_client_name text,
  p_delivery_fee numeric DEFAULT 0,
  p_delivery_address jsonb DEFAULT NULL,
  p_handoff_code text DEFAULT NULL
) RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN coalesce(nullif(btrim(p_channel), ''), 'pickup') = 'delivery' THEN 'delivery'
    WHEN coalesce(nullif(btrim(p_channel), ''), 'pickup') = 'online' THEN 'online'
    WHEN lower(btrim(coalesce(p_client_name, ''))) IN ('salón', 'salon') THEN 'mesa'
    WHEN coalesce(p_delivery_fee, 0) > 0
      OR p_delivery_address IS NOT NULL
      OR nullif(btrim(p_handoff_code), '') IS NOT NULL
      OR lower(btrim(coalesce(p_client_name, ''))) = 'delivery'
      THEN 'delivery'
    ELSE 'retiro'
  END;
$$;

DROP INDEX IF EXISTS public.orders_shift_sequence_uidx;
DROP INDEX IF EXISTS public.orders_shift_channel_sequence_uidx;

CREATE UNIQUE INDEX orders_shift_group_sequence_uidx
  ON public.orders (
    shift_id,
    public.order_sequence_group(channel, client_name, delivery_fee, delivery_address, handoff_code),
    shift_sequence
  )
  WHERE shift_id IS NOT NULL AND shift_sequence IS NOT NULL;

-- Renumerar sin colisionar con el índice único previo.
UPDATE public.orders
SET shift_sequence = shift_sequence + 100000
WHERE shift_id IS NOT NULL AND shift_sequence IS NOT NULL;
...
BEGIN
  ...
  IF v_fulfillment NOT IN ('delivery', 'salon', 'mesa') THEN
  ...
  v_sequence_group := public.order_sequence_group(
    v_channel, p_client_name, v_delivery_fee, p_delivery_address, NULL
  );
  ...
  IF v_shift_id IS NOT NULL THEN
    SELECT coalesce(max(o.shift_sequence), 0) + 1 INTO v_shift_sequence
    FROM public.orders o
    WHERE o.shift_id = v_shift_id
      AND public.order_sequence_group(
        o.channel, o.client_name, o.delivery_fee, o.delivery_address, o.handoff_code
      ) = v_sequence_group;
  END IF;
