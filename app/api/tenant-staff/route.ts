...
export async function PUT(req: NextRequest) {
	try {
		...
		const { data: userRow, error: userError } = await supabaseAdmin
			.from("users")
			.select("id,auth_id,company_id,role")
			.eq("id", id)
		...
		if (normalizeTenantRole(userRow.role) === "ceo") {
			return NextResponse.json({ error: "No se puede modificar un usuario con rol CEO" }, { status: 403 });
		}
...
export async function DELETE(req: NextRequest) {
	try {
		...
		const { data: userRow, error: userError } = await supabaseAdmin
			.from("users")
			.select("id,auth_id,email,company_id,role")
			.eq("id", id)
		...
		if (normalizeTenantRole(userRow.role) === "ceo") {
			return NextResponse.json({ error: "No se puede eliminar un usuario con rol CEO" }, { status: 400 });
		}
