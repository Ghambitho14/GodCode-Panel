...
describe("computeShiftTotals", () => {
	...
	it("counts delivery collected from sale movements with embedded order", () => {
		const totals = computeShiftTotals([
			{
				type: "sale",
				amount: 12000,
				payment_method: "cash",
				orders: { delivery_fee: 1500 },
			},
		]);
		expect(totals.deliveryCollected).toBe(1500);
		expect(totals.deliveryRefunded).toBe(0);
		expect(totals.deliveryPaidToCourier).toBe(0);
	});

	it("tracks courier payouts and delivery refunds", () => {
		const totals = computeShiftTotals([
			{
				type: "sale",
				amount: 12000,
				payment_method: "cash",
				orders: { delivery_fee: 2000 },
			},
			{
				type: "expense",
				amount: 800,
		...
		expect(totals.deliveryCollected).toBe(2000);
		expect(totals.deliveryRefunded).toBe(2000);
		expect(totals.deliveryPaidToCourier).toBe(800);
