...
export function mergeDeliverySettingsJson(
	...
	if ("ordersViewMode" in patch) {
		next.ordersViewMode = parseOrdersViewMode(patch.ordersViewMode);
...
export type OrdersViewMode = "mesas" | "pedido";

/**
 * Vista de pedidos del panel admin por sucursal (JSONB `delivery_settings`).
 * `mesas`: grilla de mesas/motos; `pedido`: tablero clásico por columnas.
 */
export function parseOrdersViewMode(raw: unknown): OrdersViewMode {
	if (raw === "pedido" || raw === "kanban") return "pedido";
	if (raw === "mesas") return "mesas";
	if (!raw || typeof raw !== "object" || Array.isArray(raw)) return "mesas";
	const o = raw as Record<string, unknown>;
	if (o.ordersViewMode === "pedido" || o.ordersViewMode === "kanban") return "pedido";
	if (o.ordersViewMode === "mesas") return "mesas";
	if (o.orders_view_mode === "pedido" || o.orders_view_mode === "kanban") return "pedido";
	if (o.orders_view_mode === "mesas") return "mesas";
	return "mesas";
}
