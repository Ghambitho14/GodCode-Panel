/**
 * Caché del catálogo del panel (categories + products de la empresa y overlay
 * por sucursal). El motor —RAM + sessionStorage, dedup en vuelo y TTL— vive en
 * `createSessionCache`, compartido con `panelDataCache`.
 */
...
const cache = createSessionCache({
	prefix: 'gc:panel:',
	defaultMaxAgeMs: COMPANY_CATALOG_MAX_AGE_MS,
	useSession: true,
});
...
export function getCompanyCatalog(companyId, fetcher, options) {
	...
	return cache.getCached(companyKey(companyId), fetcher, {
...
export function getBranchOverlay(branchId, fetcher, options) {
	...
	return cache.getCached(branchKey(branchId), fetcher, {
...
export function invalidateCompanyCatalog(companyId) {
	if (!companyId) return;
	cache.invalidate(companyKey(companyId));
}

/** @param {string | null | undefined} branchId */
export function invalidateBranchOverlay(branchId) {
	if (!branchId || branchId === 'all') return;
	cache.invalidate(branchKey(branchId));
}
