...
export default function ManualOrderE2EHarness() {
	...
	return (
		<main className="mx-auto grid min-h-screen max-w-3xl gap-5 bg-gc-page p-5 text-gc-text" data-testid="manual-order-e2e-harness">
			...
			<div className="flex flex-wrap gap-3"><button type="button" disabled={submitting || Boolean(validationMessage)} onClick={() => void submit()}>{submitting ? 'Enviando…' : mode === 'quick_sale' ? (chargeNow ? 'Cobrar y crear' : 'Crear pendiente') : fulfillment === 'table' ? 'Abrir mesa' : chargeNow ? 'Cobrar y abrir retiro' : 'Abrir retiro pendiente'}</button><button type="button" onClick={() => void saveDraft()}>Guardar borrador</button><button type="button" onClick={() => void restoreDraft()}>Restaurar borrador</button></div>
